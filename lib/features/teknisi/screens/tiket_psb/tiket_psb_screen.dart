...
import 'tiket_psb_detail_screen.dart';

class TiketPsbScreen extends StatefulWidget {
  final int initialIndex;

  const TiketPsbScreen({super.key, this.initialIndex = 1});

  @override
  State<TiketPsbScreen> createState() => _TiketPsbScreenState();
}

class _TiketPsbScreenState extends State<TiketPsbScreen>
    with SingleTickerProviderStateMixin {
  List<PsbTicket> _tickets = [];
  ...
  bool _isFinishedTicket(PsbTicket ticket) {
  ...
  bool _isActiveTicket(PsbTicket ticket, int? userId) {
  ...
  List<PsbTicket> _ticketsByFilter(String filterStatus, int? userId) {
    switch (filterStatus) {
      case 'active':
        return _tickets
  ...
  Future<void> _fetchTickets() async {
    ...
    try {
      ...
      final tickets = await ticketService.getPsbTickets();
      ...
      final filtered = keyword.isEmpty
          ...
          : tickets.where((ticket) {
              ...
              return haystacks.any((v) => v.toLowerCase().contains(keyword));
            }).toList();
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Cari tiket, pelanggan, paket...',
  ...
  Widget _buildTicketCard(PsbTicket ticket) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (isClaimable)
                const Text(
                  'Klik untuk melihat detail dan mengambil tiket ini',
                  style: TextStyle(
                    fontSize: 11,
              ...
              if (isJoinable)
                const Text(
                  'Klik untuk melihat detail dan join tiket ini',
                  style: TextStyle(
                    fontSize: 11,
